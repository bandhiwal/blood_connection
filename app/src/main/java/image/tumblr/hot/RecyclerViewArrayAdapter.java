...
public class RecyclerViewArrayAdapter<T>
        extends RecyclerView.Adapter<RecyclerViewArrayAdapter.MyViewHolder> implements Filterable{
...
*/
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        /*if (binding instanceof CategoryListitemBinding) {
            DisplayMetrics outMetrics = new DisplayMetrics();
            ((Activity)context).getWindowManager().getDefaultDisplay().getMetrics(outMetrics);

            ((CategoryListitemBinding) binding).getRoot().getLayoutParams().height = (outMetrics.widthPixels - 32)/5;
            ((CategoryListitemBinding) binding).getRoot().getLayoutParams().width = (outMetrics.widthPixels - 32)/5;
        }*/
    ...
    public int getItemViewType(int position) {
        ...
        if (mObjects.get(position) instanceof User) {
            return R.layout.item_activity_donor;
        }


        /*else if (mObjects.get(position) instanceof BusinessImage) {
>>>>>>> ceaea1f404ce1fe341f5805af4eef48d015061ce
            if (context instanceof BusinessPhotosSecondActivity) {
                return R.layout.business_photos_second_row;
            } else if (context instanceof SelectPhotos) {
                try {
                    if (((BusinessImage) mObjects.get(position)).localUri.toString().equalsIgnoreCase("CAMERA_LAYOUT")) {
                        return R.layout.business_photo_camera_bg;
                    } else {
                        return R.layout.business_photos_row;
                    }
        ...
        } else if (mObjects.get(position) instanceof Category) {

            return R.layout.list_item_sub_category;

        } else if (mObjects.get(position) instanceof Location) {

            return R.layout.view_more_locations_row;
    ...
    public void insert(final List<T> objects, int index) {
        ...
        notifyItemRangeInserted(index, getItemCount() - 1);
    }
